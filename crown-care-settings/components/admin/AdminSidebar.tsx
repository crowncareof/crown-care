...
import { FiGrid, FiLayers, FiImage, FiMessageSquare, FiLogOut, FiMenu, FiX, FiExternalLink, FiMail, FiSettings, FiUsers } from 'react-icons/fi';
...
const NAV_ADMIN = [
  ...
  { href: '/admin/leads',         label: 'Leads',         icon: FiMail,          role: 'admin' },
  { href: '/admin/settings',      label: 'Settings',      icon: FiSettings,      role: 'admin' },
  { href: '/admin/team',          label: 'Team',          icon: FiUsers,         role: 'admin' },
];

interface Props { user?: { name: string; email: string; role: string } | null }
...
export default function AdminSidebar({ user }: Props) {
  ...
  const navItems = isAdmin ? [...NAV_ADMIN.slice(0,1), ...NAV_ALL, ...NAV_ADMIN.slice(1)] : NAV_ALL;
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      <div className="px-6 py-5 border-b border-white/10">
        ...
        <div className="text-white/30 text-[10px] font-body tracking-widest uppercase mt-2">Admin Panel</div>
      </div>
          ...
          <div className="px-4 py-3 rounded-xl bg-white/5">
            ...
            <div className="flex items-center justify-between mt-0.5">
              ...
              <span className={`text-xs px-1.5 py-0.5 rounded-full flex-shrink-0 ml-1 ${
                isAdmin ? 'bg-gold-500/20 text-gold-400' : 'bg-blue-500/20 text-blue-400'
              }`}>
